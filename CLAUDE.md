...
3. **fetch_lyrics** (`fetch_lyrics.py`, `lyric_audio_match.py`, `transcribe.py`; HISTORY
   ...
   or >12 sung words the lyrics lack (backing vocals ignored; unsung lead lines dropped). None passing keeps the least-bad match flagged: no auto-upload, listed in "Flagged for Lyrics Review". Claude then
